...
class FacilityAvailabilitiesItemStruct(BaseModel):
    ...
    weekdays: Optional[Dict[str, Any]] = Field(
        None, description="Key: weekday, Value: open/close indicator"
    )
...
class Facility(BaseModel):
    ...
    facility_name: Optional[str] = Field(
        None, description="Human-readable name of the facility."
    )
    is_active: Optional[bool] = Field(
        None, description="Whether the facility is operational."
    )
    city: Optional[str] = Field(None, description="City where the facility is located.")
    postal_code: Optional[str] = Field(
        None, description="Postal or ZIP code of the facility."
    )
    geo_position: Optional[GeoPositionStruct] = Field(
        None, description="Geographic coordinates of the facility."
    )
    facility_types: Optional[List[FacilityTypesItemStruct]] = Field(
        None,
        description="List of facility type classifications (e.g., terminal, warehouse, depot, customer location).",
    )
    defined_areas: Optional[List[DefinedAreasItemStruct]] = Field(
        None, description="Sub-areas within the facility, such as gates, yards, berths."
    )
    facility_availabilities: Optional[List[FacilityAvailabilitiesItemStruct]] = Field(
        None, description="Operating time windows when the facility is available."
    )
    facility_offerings: Optional[List[FacilityOfferingsItemStruct]] = Field(
        None,
    ...
    geoid: Optional[str] = Field(
        None, description="Geographical ID for location matching."
    )
    hsud_code: Optional[str] = Field(
        None, description="HSUD code used in operational systems."
    )
