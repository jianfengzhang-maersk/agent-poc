...
class Container(BaseModel):
    ...
    container_id: str = Field(
        ..., description="Container identifier, derived from equipment_number."
    )
    sequence_number: Optional[int] = Field(
        None,
        description="Internal sequence number associated with the equipment record.",
    )
    is_active: Optional[bool] = Field(
        None, description="Whether the container is currently active in operations."
    )
    equipment_size: Optional[str] = Field(
        None, description="Container size designation (e.g., 20GP, 40GP, 40HC)."
    )
    equipment_type: Optional[EquipmentTypeStruct] = Field(
        None, description="Container type classification hierarchy."
    )
    ownership: Optional[OwnershipStruct] = Field(
        None, description="Ownership and leasing details for the container."
    )
    lifecycle: Optional[LifecycleStruct] = Field(
        None, description="Manufacturing and lifecycle-related timestamps."
    )
    inspections: Optional[InspectionsStruct] = Field(
        None, description="CSC/PMI inspection details."
    )
    modification: Optional[ModificationStruct] = Field(
        None, description="Container modification or repair program information."
    )
    manufacturer_info: Optional[ManufacturerInfoStruct] = Field(
        None, description="Information about the container manufacturer and production."
    )
    weight: Optional[WeightStruct] = Field(
        None, description="Container weight specifications."
    )
    last_event_timestamp: Optional[datetime.datetime] = Field(
        None, description="Last known operational event timestamp for the container."
    )
